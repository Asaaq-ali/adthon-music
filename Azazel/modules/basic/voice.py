...
async def voice(client: Client, message):
    ...
    if len(cmd) > 1:
        ...
        await edit_or_reply(
            message,
            "**قم بالرد على الرسائل أو إرسال الوسائط النصية لتحويلها إلى صوت**",
        )
        return
...
async def voicelang(client: Client, message: Message):
    ...
    try:
        ...
        await edit_or_reply(message, "`الرجاء إدخال رمز اللغة`")
        lang = temp
        return
    await edit_or_reply(
        message, "**تم تغيير لغة Google Voice إلى** `{}`".format(lang)
    )
...
add_command_help(
    "صوتي",
    [
        [f"تتس [بالرد]", "تحويل النص الى صوت من جوجل."],
        [
            f"ضبط (رمز اللغة) ",
