...
async def _(client, message):
    ...
    TM = await message.reply("انتظر لحظة")
    if len(message.command) < 3:
        ...
        if message.command[2] in results:
            ...
            return await TM.edit(
                f"❌ msg_filter {message.command[2]} لا يمكن معالجتها\n\n✅ msg_filter ما هو متاح: <code>dokumen</code> <code>photo</code> <code>audio</code> <code>video</code>"
            )
    await TM.edit("يعالج")
    try:
        done = 0
...
add_command_help(
    "Spesial",
    [
        [f"take [foto/video][jumlah][username grup(contoh : adthon_spp)]", "Mengambil Pesan Dari Grup/Channel."],
    ],
)"""
