...
async def user_exec(client: Client, message: Message):
    if message.from_user.id not in ADMINS:
        return await message.edit("**لست مشرف**")
        
  
    if len(message.command) == 1:
        return await message.edit("<b>لم يتم توفير رمز للتنفيذ</b>")
...
async def evaluation_cmd_t(client, message):
    if message.from_user.id not in ADMINS:
        return await message.edit("*لستا مشرف**")

    status_message = await message.edit("`تقييم المعالجة..`")
    try:
        cmd = message.text.split(" ", maxsplit=1)[1]
    except IndexError:
        return await status_message.edit("`اعطاء الاوامر !`")
